...
function lib_data_xml(data_src_path, data_src_params, defaultParentStorage, global_setup)
{
  ...
  this.req_all_parents = lib_data_xml_req_all_parents.bind(this);
  this.get_all_parents = lib_data_xml_get_all_parents.bind(this);
...
function lib_data_xml_get_explorer_path(iparams)  // iparams = {elem_id, lock_id}
{
  ...
  if (my_parents.length > 0)
  {
    ...
    {   
      ...
      explorer_path[retval_ct].eval = this.get_tree_item_field(elem_id, "eval");       
...
function lib_data_xml_get_tree_nodes(iparams)  // iparams = {elem_id, explorer_path}
{
  ...
  else  // element == this.data_root_id
  {
    ...
    tree_nodes[retval_ct].eval = this.get_tree_item_field(iparams_cp.elem_id, "eval");    
    tree_nodes[retval_ct].description = unescape(this.get_tree_item_field(iparams_cp.elem_id, "content"));
  ...
  do
  {
    ...
    for(var a=0; a<my_parents.length; a++)
    {
      ...
      for(var b=0; b<curr_item_child_ids.length; b++)
      {
        ...
        tree_nodes[my_idx].eval = this.get_tree_item_field(curr_item_child_ids[b], "eval");
...
//
{
  ...
  if ((iparams_cp.favIds.length > 0) && ( (iparams.mode == "load_all") || (iparams.mode == "fav_only") ))
  {
    for (var i=0; i<iparams_cp.favIds.length; i++)
    {
      if (iparams_cp.favIds[i] == null)
      {
        ...
        this.rts_ret_struct.fav[i].eval = jQuery.extend(true, [], this.empty_eval_struct);
      }
      else
      { 
  ...
  if (iparams.mode == "load_all")
                                        // The "load all" option is usually loaded while the "curr_uc_main" 
                                        // is being created. Since the object is marked as undefined in the 
                                        // global_main unless its init function has finished and 
                                        ...
                                        // main object. A value of 100ms should halt the callback function until
...
function lib_data_xml_req_all_parents(iparams)
{               
  this.currElem = iparams.elem_id;
  eval(iparams.cb_fctn_str); 
}

function lib_data_xml_get_all_parents(itemId)
{
  itemId = this.currElem;
  var parent_ids = this.get_tree_item_parents(itemId);
  var retlist = [];
  if (parent_ids.length > 0)
  {
    ...
    { 
      retlist[i] = {};
      currItem = getDBElementById(this.db_buffer, parent_ids[i]);
      var myItemField = currItem.getElementsByTagName("name");
      if (myItemField.length != 0)
      {
        retlist[i].name = getInnerHTML(myItemField[0]);
      }      
      retlist[i].elem_id = parent_ids[i];
    }
  }
  return retlist;
}
...
function lib_data_xml_create_tree_item( iparams )  // iparams = {parent_elem_id, name, type, lock_id, cb_fctn_str}  
{
  ...
  if (parentItem != undefined)
  {
    ...
    var evalField = this.db_buffer.createElement("eval");
      for (var i=0; i<this.eval_cat_num; i++)
      {        
    ...
    currItem.appendChild(evalField); 
...
function lib_data_xml_change_tree_item_field(iparams)      // iparams = {items, field_id, content, lock_id, cb_fctn_str}
{
  ...
  if (currItem != undefined) 
  {
    ...
    {
      switch (iparams_cp.field_id)
      {
        case "content" : 
            setInnerHTML(myItemField[0], escape(iparams_cp.content));
          break;
        ...
        case "eval" :
            ...
            for (var i=0; i<this.eval_cat_num; i++)
            { 
              var myCatResults = myItemField[0].getElementsByTagName(c_LANG_UC_BROWSING_PANEL2_EVAL_CATS[i+1][0]);
              myCatField[i] = myCatResults [0];
                                  // category already exists -> fill (and repair)
              if (myCatField.length > 0)
              {
                if (myCatField[i] != undefined)
                {
                  // "avg" field
                  var myAvgField = myCatField[i].getElementsByTagName("avg");
                  if (myAvgField.length > 0)
                  {
                    setInnerHTML(myAvgField[0], iparams_cp.content[i].avg);
                  }
                  else                  
                  {
                    myAvgField[0] = this.db_buffer.createElement("avg");
                    setInnerHTML(myAvgField[0], String(iparams.content[i].avg));
                    myCatField[i].appendChild(myAvgField[0]);
                  }
                  
                  // "num" field
                  var myNumField = myCatField[i].getElementsByTagName("num");
                  if (myNumField.length > 0)
                  {
                    setInnerHTML(myNumField[0], iparams_cp.content[i].num);
                  }
                  else                  
                  {
                    myNumField[0] = this.db_buffer.createElement("num");
                    setInnerHTML(myNumField[0], String(iparams.content[i].num));
                    myCatField[i].appendChild(myNumField[0]);
                  }
                }
                else
                {
                  myCatField = this.db_buffer.createElement(c_LANG_UC_BROWSING_PANEL2_EVAL_CATS[i+1][0]);
                  var myAvgField = this.db_buffer.createElement("avg");
                  setInnerHTML(myAvgField, String(iparams.content[i].avg));
                  var myNumField = this.db_buffer.createElement("num");
                  setInnerHTML(myNumField, String(iparams.content[i].num));  
                  myCatField.appendChild(myAvgField);          
                  myCatField.appendChild(myNumField);    
                  myItemField.appendChild(myCatField);    
                }
...
function lib_data_xml_get_tree_item_field(itemId, fieldId)
{
  ...
  if (currItem != undefined) 
  {
    ...
    if (myItemField.length != 0)
    {
                                    // evaluation values are treated differently
      if (fieldId == "eval")
      {
                                    // create empty eval structure
        var my_eval = jQuery.extend(true, [], this.empty_eval_struct);
                                    // traverse all categories
        for (var i=0; i<this.eval_cat_num; i++)
        {
          var myCatField = myItemField[0].getElementsByTagName(c_LANG_UC_BROWSING_PANEL2_EVAL_CATS[i+1][0]);
                                    // if category exists ...
          if (myCatField.length != 0)
          {
                                    // ... and average vote score entry exists ... 
            var my_avg_val = myCatField[0].getElementsByTagName("avg");
            if (my_avg_val.length != 0)
            { 
                                    // ... extract it
              my_eval[i].avg = parseFloat(getInnerHTML(my_avg_val[0]));
            } 
                                    // ... and number of votes exists ... 
            var my_num_val = myCatField[0].getElementsByTagName("num");
            if (my_num_val.length != 0)
            { 
                                    // ... extract it
              my_eval[i].num = parseInt(getInnerHTML(my_num_val[0]));
            } 
          }
        }
        return my_eval;
